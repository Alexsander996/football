...
На основе этой диаграммы разработывается DFD - диаграмму [5] (смотри рис.2).
...
Белым цветом отмечены сущности или хранилища, на их основе составляется следующая диаграмма - ER - диаграмма [6] (смотри рис.3).
...
На основе этой диаграммы создаются классы в среде C# и описываются с параметрами и их типами данных (смотри рис.4)
...
Создаются такие классы для всех сущностей из ER - диаграммы (смотри рис.5)
...
После создается хранилище для класса (смотри рис.6).
...
Таким же образом создаются хранилища для остальных классов (смотри рис.7).
...
Далле создаются контроллеры с методами CRUD [7] (смотри рис.8 и рис.9).
...
После запуска программы открывается страница Swagger UI со списком сущностей и операций над ними (смотри рис.10).
...
Проверка работы методов CRUD в сущности Player (смотри рис.11).
...
1. Create позволяет добавлять новые строки в вашу таблицу (смотри рис.12).
2. 
...
Ввод необходимых данных об игроке в соответсвующие поля (смотри рис.13).
...
После успешного выполнения оперции ответ сервера выглядит так (смотри рис.14). 
...
2. Операция чтения Read извлекает определенные записи и считывать их значения (смотри рис.15).
...
Считывание информации об уже соззданном игроке по его ID (смотри рис.16)
...
После выполнения операции сервер выдает информацию о игроке, которая была создана в Create (смотри рис.17).
...
3. Функция Update редактирует существующие записи. Используется для изменения записей в базе данных (смотри рис.18).
...
Чтобы изменить информацию о игроке, нужно сначала ввести его ID и потом новые данные (смотри рис.19).
...
После выполнения операции сервер выдает новую информацию о игроке с ID = 1 (смотри рис.20).
...
Операцией Read можно проверить результат вышеуказанной операции (смотри рис.21).
...
Удаление игрока по его ID (смотри рис.22).
...
После выполнения операции ответ сервера будет (смотри рис.23).
